/**
 * FloatingWaves — лёгкий фоновый компонент анимированных волн на Canvas 2D.
 *
 * Без внешних библиотек. Изолирован: ничего не знает о остальном приложении,
 * получает только canvas-элемент и (опционально) базовый цвет.
 *
 * Особенности:
 *  - 4 слоя волн с собственными amplitude / frequency / phase / speed;
 *  - плавная заливка через bezierCurveTo для органичной формы;
 *  - лёгкая реакция на курсор (локальный подъём ближайшей к курсору волны);
 *  - devicePixelRatio-aware рендер (чёткость на Retina);
 *  - пауза при скрытой вкладке или когда canvas вне вьюпорта (IntersectionObserver);
 *  - ResizeObserver для корректной адаптации под размер контейнера;
